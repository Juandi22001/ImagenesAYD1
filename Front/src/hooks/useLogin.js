...
export const useLogin = () => {
  ...
  const Login = async (Usuario, Password) => {
   ...
   await axios.post('http://localhost:5000/login', usuario )
   .then(response => {
    ...
    if(result.rol==0){
      window.location.replace("http://localhost:3000/principal_admin");
      localStorage.setItem('usuario',JSON.stringify(result))


    }  if(result.rol==1){
      window.location.replace("http://localhost:3000/crear_vuelos");
      localStorage.setItem('usuario',JSON.stringify(result))


    }
    if(result.rol==2){
      window.location.replace("http://localhost:3000/crear_habitaciones");
      localStorage.setItem('usuario',JSON.stringify(result))
    ...
    if(result.rol==3){
      window.location.replace("http://localhost:3000/crear_carrito");
      localStorage.setItem('usuario',JSON.stringify(result))
